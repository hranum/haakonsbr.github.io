 ...
 */
const fetchData = async (url, opts = {}, retryAttempts = 0, timeStarted = 0) => {
  ...
  const maxRetryAttempts = 1; // Maximum request attempts
  ...
  try {
    ...
    return await fetchData(url, opts, retryAttempts, timeStarted);
  }
};
...
const getStationInformation = async () => {
  ...
  const stationCache = readCache(cacheKey);
  if (!stationCache) {
    const stations = await fetchData('station_information.json');
    if (stations && stations.data && stations.data.data && Array.isArray(stations.data.data.stations)) {
      writeCache(cacheKey, stations.data.data.stations, ttl);
...
const getStationStatuses = async () => {
  ...
  if (!stationStatusCache) {
    const stationStatuses = await fetchData('station_status.json');
    if (stationStatuses && stationStatuses.data && stationStatuses.data.data &&
      ...
      writeCache(cacheKey, stationStatuses.data.data.stations, ttl);
